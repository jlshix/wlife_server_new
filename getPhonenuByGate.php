...
if(isset($_POST['gate']) && isset($_POST['housenumber'])){
    $housegate = $_POST['gate'] ;
    $houseNumber = $_POST['housenumber'] ;
    $sql_getImei = "SELECT `gimei` FROM `houseGate` WHERE `bulidingImei`={$housegate} AND `housenu`={$houseNumber}" ;
//  $sql_test = "SELECT `gimei` FROM `houseGate` WHERE `bulidingImei`='12345' AND `housenu`='502'" ;
    ...
    if($stmt_imei->rowCount() == 1){
        $gimei = $res_imei[0] ;
        $sql_getPhone = "SELECT `phone` FROM `phoneGate` WHERE `gimei`={$gimei} AND `status`='1' LIMIT 0,1" ;
